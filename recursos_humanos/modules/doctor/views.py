...
class DoctorViewSet(BaseModelViewSet):
    ...
    def create(self, request, result_serializer, *args, **kwargs):
        with transaction.atomic():
            username = "slg_" + result_serializer.data[STRING(Doctor.dni)][:2]
            contrasenia = result_serializer.data[STRING(Doctor.dni)]
            ...
            doctor = Doctor(
                nombres=result_serializer.data[STRING(Doctor.nombres)],
                apellidos=result_serializer.data[STRING(Doctor.apellidos)],
                dni=result_serializer.data[STRING(Doctor.dni)],
                celular=result_serializer.data[STRING(Doctor.celular)],
                fechaNacimiento=result_serializer.data[STRING(Doctor.fechaNacimiento)],
                usuario_id=user.id,
            )
    ...
    def update(self, request, result_serializer, *args, **kwargs):
        with transaction.atomic():
            instance = self.get_object()
            if result_serializer.data.get(STRING(Doctor.ubicaciones)) is not None:
                instance.ubicaciones.set(
                    result_serializer.data[STRING(Doctor.ubicaciones)]
                )
            instance.nombres = result_serializer.data[STRING(Doctor.nombres)]
            instance.apellidos = result_serializer.data[STRING(Doctor.apellidos)]
            instance.dni = result_serializer.data[STRING(Doctor.dni)]
            instance.celular = result_serializer.data[STRING(Doctor.celular)]
            instance.fechaNacimiento = result_serializer.data[
                STRING(Doctor.fechaNacimiento)
            ]
